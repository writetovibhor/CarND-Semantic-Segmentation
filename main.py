...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    def conv1x1(in_layer):
        return tf.layers.conv2d(in_layer, num_classes, 1,
            padding = 'same',
            kernel_initializer = tf.random_normal_initializer(stddev=0.01),
            kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))

    def upsample(in_layer, depth, strides):
        return tf.layers.conv2d_transpose(in_layer, num_classes, depth,
            strides = strides,
            padding = 'same',
            kernel_initializer = tf.random_normal_initializer(stddev=0.01),
            kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    ...
    upsample1 = upsample(layer7_1x1, depth=4, strides=(2, 2))
    skip1 = tf.add(layer4_1x1, upsample1)
    upsample2 = upsample(skip1, depth=4, strides=(2, 2))
    skip2 = tf.add(layer3_1x1, upsample2)
    upsample3 = upsample(skip2, depth=16, strides=(8, 8))
    return upsample3
